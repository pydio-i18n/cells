...
func NewRouterEventFilter(options RouterOptions) *RouterEventFilter {
	...
	handlers = append(handlers,
		...
		&PutHandler{},        // Client adding a node precreation on PUT file request
		&EncryptionHandler{}, // Client retrieve encryption materials from encryption service
		&VersionHandler{},
		&Executor{},
	)
	...
	r.handlers = handlers
	r.pool = pool
	r.rootsCache = cache.New(120*time.Second, 10*time.Minute)
	r.initHandlers()
	return r
