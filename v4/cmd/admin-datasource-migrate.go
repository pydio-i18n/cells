	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		*/
		...
		if len(hiddenNodes) > 0 && !migrateDry {
			...
			if _, e := p.Run(); e == nil {
				if tgtFmt == "flat" {
					resyncClient := sync.NewSyncEndpointClient(grpc.NewClientConn(common.ServiceDataIndex_ + source.Name))
...
func migratePrepareClients(source *object.DataSource) (rootNode *tree.Node, idx tree.NodeProviderClient, mc nodes.StorageClient, e error) {
	...
	idx = tree.NewNodeProviderClient(grpc.NewClientConn(common.ServiceDataIndex_ + source.Name))
	...
	rootNode = r.GetNode()
	objCli := object.NewObjectsEndpointClient(grpc.NewClientConn(common.ServiceDataObjects_ + source.ObjectsServiceName))
