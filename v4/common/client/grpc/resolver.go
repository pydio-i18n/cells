...
func (cr *cellsResolver) sendState() {
	...
	for _, v := range cr.items {
		...
		if v.As(&svc) {
			for _, node := range svc.Nodes() {
				m[node.ID()].services = append(m[node.ID()].services, svc.Name())
			}
		}
	}
	cr.ml.RUnlock()
	...
	if err := cr.cc.UpdateState(resolver.State{
		Addresses:     addresses,
		ServiceConfig: cr.cc.ParseServiceConfig(`{"loadBalancingPolicy": "lb", "health_check_config"": {"serviceName": "cells"}`),
	}); err != nil {
