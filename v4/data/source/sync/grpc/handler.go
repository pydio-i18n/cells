...
import (
	...
	protoservice "github.com/pydio/cells/v4/common/proto/service"
	...
	go func() {
		...
		std.Retry(ctx, func() error {
			...
			if syncConfig.ObjectsBucket == "" {
				_, err = oc.ListBucketsWithContext(testCtx)
				if err != nil {
					if retryCount > 1 {
				...
				_, err = oc.ListObjectsWithContext(testCtx, syncConfig.ObjectsBucket, "", "/", "/", 1)
				if err != nil {
					if retryCount > 1 {
